...
export default async function editDashboardUseCase(
  ...
  const dashboard = await context.dashboardRepository.getDashboardById(id as string);
  ...
  if (dashboard.user.id !== context.user.id) {
    throw new Error(`User ${context.user.id} is not allowed to edit dashboard ${id}`);
  }
